...
```mermaid
flowchart LR
  A[Age] --> I[Thymic involution]
  I --> T[Reduced/altered T-cell output]
  T --> V[Infection and cancer vulnerability]
  GH[Growth-factor regimen] --> R[Regrowth]
  R --> T
  GH --> C[Tumor growth / metabolic risk]
  R --> U[Possible faulty selection]
  U --> AI[Autoimmunity]
```
