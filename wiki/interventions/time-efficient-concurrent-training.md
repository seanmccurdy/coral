...
```mermaid
flowchart TD
  TIME{Weekly time available} -->|about 3 hours| MIX[Allocate both resistance and aerobic work]
  MIX --> RT[2-3 whole-body resistance sessions]
  MIX --> HI[1-2 high-intensity interval sessions]
  MIX --> LI[1-2 low-intensity movement sessions]
  RT --> MUS[Strength and lean-tissue stimulus]
  HI --> VO2[VO2 max and vascular stimulus]
  LI --> BASE[Aerobic base, movement and recovery]
  LOWE[Low energy / novice / poor recovery] --> ADJ[Reduce vigorous dose or volume]
  ADJ --> MIX
  GOAL{Goal and response} --> ADJ
```
