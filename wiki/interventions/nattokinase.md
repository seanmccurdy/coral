...
```mermaid
flowchart LR
  NK[Nattokinase] --> FIB[Greater fibrinolytic / anticoagulant activity]
  FIB -->|potential benefit| THR[Less pathological thrombosis]
  THR --> EVT[Fewer ischemic events?]
  FIB -->|potential harm| HEM[Impaired hemostasis]
  HEM --> BLEED[Bleeding events?]
  NK --> BP[Possible lower systolic pressure]
  NK --> LIP[Inconsistent lipid / glucose changes]
  BP --> EVT
  LIP --> PLAQUE[Plaque change?]
  PLAQUE --> EVT
```
