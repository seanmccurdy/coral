...
```mermaid
flowchart TD
  A[Symptoms, age, time since menopause, goals] --> C[Contraindications and baseline risk review]
  C --> U{Uterus present?}
  U -->|yes| EP[Systemic estrogen plus adequate endometrial protection]
  U -->|no| E[Estrogen may be used without progestogen]
  A --> L{Only genitourinary symptoms?}
  L -->|yes| V[Consider local vaginal therapy]
  EP --> R[Reassess symptoms, bleeding, adverse effects, risk]
  E --> R
  V --> R
  R --> D[Adjust, continue, or stop through shared decision]
```
