...
```mermaid
flowchart LR
  SUPP[Creatine supplementation] --> POOL[Muscle phosphocreatine pool]
  POOL --> ATP[Rapid ATP regeneration]
  ATP --> OUTPUT[Higher-quality high-intensity sets]
  OUTPUT --> TRAIN[Greater training stimulus]
  TRAIN --> ADAPT[Strength, power, muscle mass]
  SUPP --> WATER[Intracellular water]
  WATER --> ADAPT
  SUPP -. mixed trial evidence, unresolved mechanism .-> MOOD[Depression outcomes]
```
