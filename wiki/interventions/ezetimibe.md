...
```mermaid
flowchart LR
  LIVER[Liver cholesterol] --> BILE[Biliary cholesterol in intestine]
  FOOD[Dietary cholesterol] --> LUMEN[Intestinal lumen]
  BILE --> LUMEN
  NPC[NPC1L1 transporter] --> ABS[Cholesterol absorption]
  LUMEN --> NPC
  EZE[Ezetimibe] -->|blocks| NPC
  ABS --> HEP[Hepatic cholesterol pool]
  HEP -->|reduces receptor demand| LDLR[LDL-receptor clearance]
  EZE -->|lower hepatic pool increases| LDLR
  STATIN[Statin] -->|reduces synthesis| HEP
  LDLR -->|lowers| LDL[Circulating LDL / ApoB exposure]
  LDL --> PLAQUE[Atherosclerotic events over time]
```
