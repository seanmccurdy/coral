...
```mermaid
flowchart TD
  DEMAND[Ventilatory demand and arousal] --> PATTERN{Breathing strategy}
  PATTERN --> Z1[Zone 1: diaphragm descent and abdominal expansion]
  PATTERN --> Z2[Zone 2: diaphragm plus rib/intercostal motion]
  PATTERN --> Z3[Zone 3: face, neck and shoulder accessory effort]
  Z1 --> EFF[Low-effort ventilation]
  Z2 --> EFF
  Z3 -->|brief high demand| PERF[Emergency or finish-line ventilation]
  Z3 -->|persistent| LOAD[Accessory-muscle load and guarding]
  LOAD --> SYM[Possible pain or tension]
  TOUCH[Touch/pressure feedback] --> AWARE[Awareness of regional motion]
  SLOW[Slow paced breathing] --> AROUSAL[Lower short-term arousal]
  AWARE --> PRACTICE[Practice distributing breath]
  PRACTICE --> EFF
```
...
```mermaid
flowchart LR
  P1[Perspective] --> SESSION[Five-minute practice]
  P2[Place] --> SESSION
  P3[Supported position] --> SESSION
  P4[Slow breath pace] --> SESSION
  P5[Gentle palpation] --> SESSION
  SESSION --> FEEDBACK[Notice breath effort, tension and affect]
  FEEDBACK --> ADJUST[Reduce effort or stop if symptomatic]
  ADJUST --> REPEAT[Repeat and assess function over time]
```
...
```mermaid
flowchart LR
  P[Pressure and rising arousal] --> B[Slow paced breathing]
  B --> R[Respiratory heart-rate oscillation]
  R --> D[Live HRV display]
  D --> L[Learn which pace and effort produce a stable response]
  L --> W[Withdraw immediate feedback]
  W --> C[Use a simple count or internal cue under pressure]
  C --> A[More composed next action]
  A --> F[Later review and refinement]
```
