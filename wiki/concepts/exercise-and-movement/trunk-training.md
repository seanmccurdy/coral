...
```mermaid
flowchart TD
  LOAD[External load or intended movement] --> TASK{Required trunk function}
  TASK --> FLEX[Produce flexion]
  TASK --> ROT[Produce rotation]
  TASK --> AEXT[Resist extension]
  TASK --> AROT[Resist excess rotation]
  TASK --> ALAT[Resist lateral flexion]
  FLEX --> CAP[Task-specific force and control]
  ROT --> CAP
  AEXT --> CAP
  AROT --> CAP
  ALAT --> CAP
  CAP --> MOVE[Transfer force while controlling spinal position]
```
...
```mermaid
flowchart LR
  ISO[1 Isometric position<br/>fatigue resistance] --> ECCF[2 Anti-movement or eccentric<br/>fatigue resistance]
  ECCF --> ECCL[3 Anti-movement or eccentric<br/>heavy load]
  ECCL --> CONF[4 Concentric movement<br/>fatigue resistance]
  CONF --> CONL[5 Concentric movement<br/>heavy load]
  TECH{Technique stable?} -->|yes| NEXT[Advance one demand]
  TECH -->|no| REG[Regress load, range or fatigue]
```
