...
```mermaid
flowchart TD
  POS[Positive amyloid blood test] --> Q{Symptomatic or high pretest risk?}
  Q -->|no symptoms, no family history| CAUTION[Do not diagnose; consider not testing at all]
  Q -->|surprising positive| CONFIRM[Confirm: amyloid + tau PET or CSF]
  Q -->|strong family history / APOE4-4| PRECLIN[Preclinical workup can be justified]
  CONFIRM -->|amyloid only, no tau, no deficit| WAIT[Amyloid-positive, not clinical AD - IWG view]
  CONFIRM -->|amyloid + tau + deficit| DX[Alzheimer's disease - both camps agree]
  POS -->|AA biological criteria| DXBIO[Alzheimer's diagnosable from amyloid alone]
  DXBIO -.contested.-> WAIT
  PRECLIN -->|amyloid present| RX[Consider preventive treatment - contested frontier]
```
